...
export const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const ownerid = await getOwnerID(email,password);
  
      const credentials = {
        ownerid:ownerid,
      };
      const response = await fetch('http://localhost:5000/saveCredentialsOwner', {
        method: 'POST',
        headers: {
  ...
  const getOwnerID = async (email,password) => {
    try {
